...
/**
 * Displays the drip schedule form fields on the subscription level edit screen.
 */
...
?>
	<tr class="form-field" id="rcp-lqa-wrapper">
		<th scope="row" valign="top">
			<label for="rcp-lqa"><?php _e( 'Total Quantity Available', 'rcp-lqa' ); ?></label>
		</th>

		<td>
			<input type="number" id="rcp-lqa-total-available" name="rcp-lqa-total-available" value="<?php echo esc_attr( $qty ); ?>" style="width: 60px;">
			<p class="description"><?php _e( 'Limit the total number of times this subscription level can be sold. For example, enter 20 to allow a maximum of 20 sales. After 20 sales, the subscription level will not show on your registration form. Renewal payments do not count towards total sales. Enter 0 for unlimited. If you set up limited quantities on a subscription level that already has sales, those sales will count towards the total.', 'rcp-lqa' ); ?></p>
		</td>
	</tr>
	<?php
...
/**
 * Saves the drip schedule defined on the subscription edit screen.
 */
